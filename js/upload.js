...
async function cargarArchivos(semana) {
  ...
  data.forEach((file) => {
    ...
    // 📅 Generar fecha "fake" si no existe created_at (usamos timestamp dentro del nombre)
    let fecha = "—";
    ...
    row.innerHTML = `
      ...
      <td>${fecha}</td>
      <td>
        <a href="${publicUrl}" target="_blank" class="btn btn-success btn-sm me-2">👁️ Ver</a>
        <button class="btn btn-danger btn-sm" onclick="borrarArchivo('${normalizar(
          semana
